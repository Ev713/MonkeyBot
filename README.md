...
In contrast to most wall-climbing robotics, MonkeyBot tackles the challenge of **sparse, discrete, and spatially constrained gripping points**—a scenario common in human or natural climbing. The system models the climbing process as a **numeric planning problem** that integrates discrete grip selection with complex, continuous body motions, such as dynamic jumps.
...
```
plans/
├── scenario_wall_01_plan.json        # High-level action sequence
├── scenario_wall_01_trajectory.csv   # Detailed position/velocity data
└── scenario_wall_01_metrics.json     # Performance metrics
```
...
- **`instance_generator.py`** — Generate new climbing scenarios with custom grip layouts
- **`angle_data_analyzer.py`** — Analyze joint angle distributions from completed runs
- **`pruning_analyzer.py`** — Evaluate planning efficiency and search space reduction
